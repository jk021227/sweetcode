...
const InterviewIsle = () => {
  ...
  return (
    <div>
      <Nav titleText="Interview Isle"></Nav>
      <div className="p-4 relative my-8 mx-auto w-[600px] rounded border-2 border-black bg-white">
        {' '}
        {/* Added relative positioning for log */}
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <div className="mb-4">
            ...
            <p className="mt-1 p-2 font-bold">{question}</p>
          </div>
          <div className="mb-4">
            <label
        ...
        {showLog && (
          <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white w-3/4 max-h-[600px] p-4 rounded-md overflow-y-auto">
              {' '}
              {/* Adjust max height to match the interview box */}
              <h2 className="text-xl font-bold mb-4">Interaction Log</h2>
              {log.map((entry, index) => (
                <div
                  key={index}
                  className={`mb-4 ${entry.question ? 'mb-4' : 'mb-0'}`}
                >
                  {' '}
                  {/* Adjust margin based on presence of question */}
                  {entry.question && (
                    <p>
                  ...
                  <p>
                    <strong>Your Answer:</strong> {entry.userInput}
                  </p>
                  <p>
                    <strong>Tutor Response:</strong> {entry.tutorResponse}
                  </p>
